...
# Sergey Tamkovich was hired by Aleph Communications to add the import ability.
#
...
sub initialize() {
	...
	$enh_config = &load_config_reseller($reseller) if ($reseller);
...
if ( $params->{reseller} ne "null" && $params->{reseller}) {
	my $config_reseller = &load_config_reseller( $params->{reseller} );
...
################ Refill ASTPP Users Account ########################
	...
	while ( $count < $params->{quantity} ) {
		$count++;
		$status .=
		  &refill_account( $astpp_db, $params->{user},
			$params->{amount} );
		&email_refill_account($astpp_db,'', $config, $params, $enh_config);
	}
	print $status;
...
#################  Calling Card Support Starts Here ################
	...
	while ( $count < $params->{quantity} ) {
		$count++;
		my ( $cc, $pin ) =
		  &add_callingcard( $astpp_db, $config, $enh_config, $branddata,
			$params->{status}, $params->{value}, $params->{user},
			$params->{pins} );
		...
		&email_add_callingcard($astpp_db,'', $config, $params, $enh_config, $cc, $pin );
	}
}
elsif ( $params->{function} =~ /refill_cc/ ) {
	print STDERR gettext("REFILLING CALLING CARD");
	$status = &transfer_funds($astpp_db,$params->{emptycard}, $params->{emptycardpin}, $params->{destcard}, $params->{destcardpin});
        if ($status == 0) {
