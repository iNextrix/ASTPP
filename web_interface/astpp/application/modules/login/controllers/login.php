		...
		if ($this->session->userdata ( 'user_login' ) == FALSE) {
			if (! empty ( $_POST ) && trim ( $_POST ['username'] ) != '' && trim ( $_POST ['password'] ) != '') {
				...
				if ($user_valid == 1) {
					...
					$data ['astpp_notification'] = "Login unsuccessful. Please make sure you entered the correct username and password, and that your account is active";
				}
			} else {
				if (! empty ( $_POST ) && ($_POST ['password'] == '' || $_POST ['username'] == '')) {
