...
class Account_import extends MX_Controller {
	...
	function customer_import_data(){
		...
		if(!empty($csv_tmp_data)){
			...
			$balance = $default_fields['posttoexternal'] == 0 && $default_fields['balance'] > 0 ? $this->common_model->add_calculate_currency ( $default_fields['balance'], '', '', false, false ) : 0 ;
			// Set credit limit after currency conversion
			$credit_limit = $default_fields['posttoexternal'] == 1 && $default_fields['credit_limit'] > 0 ? $this->common_model->add_calculate_currency ( $default_fields['credit_limit'], '', '', false, false )  : 0 ;
			foreach ( $csv_tmp_data as $key => $csv_data ) {
				$error=null;
				$current_number = !empty($add_array["number-select"]) ? $csv_data [$add_array["number-select"]] : '';
				$current_email =  !empty($add_array["email-select"]) ? $csv_data [$add_array["email-select"]]: '';
			...
			$this->session->set_flashdata ( 'astpp_errormsg','Total ' .$i. ' Accounts imported successfully!'  );
		}else{
			...
			foreach($first_value as $key=>$value){
				...
				if($value == "sip_username" || $value =="sip_password"){
					...
					if($value == "telephone_1"){
						$params_arr['placeholder'] = " Ex: 917929700670/18555801802";
					}
					if($value == "telephone_2"){
							$params_arr['placeholder'] = " Ex: 919724525224";
					}
					if($value == "email"){
							$params_arr['placeholder'] = "Ex: sales@inextrix.com";
					}
					
					if($value == "dialed_modify"|| $value == "std_cid_translation" || $value == "did_cid_translation"){
							$params_arr['placeholder'] = 'Ex: 00/,91/,1/';
					}
					if($value == "maxchannels" || $value == "cps"){
						$params_arr['placeholder'] = 'Ex: 1';
					}
