...
class dashboard extends CI_Controller {
	...
	function customerReport_call_statistics_with_profit() {
		...
		foreach ( $resellerresult->result_array () as $data ) {
			...
			$reseller_total_result ['debit'] += $acc_arr [$data ['day']] ['debit'];
			$reseller_total_result ['cost'] += $acc_arr [$data ['day']] ['cost'];
		...
		if (! empty ( $acc_arr )) {
			foreach ( $daterange as $date ) {
				...
				if (isset ( $acc_arr [$day] )) {
					...
					$json_data ['total'] [] = array (
							( string ) $acc_arr [$day] ['day'],
							( int ) $acc_arr [$day] ['sum'] 
					);
					$json_data ['answered'] [] = array (
							( string ) $acc_arr [$day] ['day'],
							( int ) $acc_arr [$day] ['answered'] 
					);
					$json_data ['failed'] [] = array (
							( string ) $acc_arr [$day] ['day'],
							( int ) $acc_arr [$day] ['failed'] 
					);
					$json_data ['profit'] [] = array (
							( string ) $acc_arr [$day] ['day'],
							( float ) str_replace ( ",", "", $this->common_model->calculate_currency ( $acc_arr [$day] ['profit'] ) ) 
					);
					$json_data ['acd'] [] = array (
							( string ) $acc_arr [$day] ['day'],
							( float ) $acd 
