...
for (l = 1; l <= cnt; l++) {
         jQuery("#auto_invoice_date_"+l).datetimepicker({format:'Y-m-d h:m:i'});		
...
<section class="slice color-three padding-b-20">
  ...
  <div class="col-md-12" style="border: 1px solid #ccc; border-radius: 5px;">    
            ...
            <form name="payment_form" id="payment_form" action="/invoices/invoice_automatically_payment_edit_save/" method="POST">
	<br/>
	<input type='hidden' name="row_count" value="<?= $row_count ?>">
	<input type='hidden' name="taxes_count" value="<?= $taxes_count; ?>">

	<input type='hidden' name="count" value="<?= $count; ?>">
	<input type='hidden' name="auto_count" value="<?= $auto_count; ?>">

	<input type="hidden" id="auto_sum_input" name="auto_sum_input" value="<?= $invoice_auto_res; ?>">
  ...
  <table class="invoice_table" width="100%" border="0" >
	...
	<!--	<tr>
			<td>
		 		 <b>Address : </b><span style="color:#a09d9d;"><?php echo $accountdata['address_1']; ?> <?php echo $accountdata['address_2']; ?></span>
		 </td>
		</tr>-->
...
<table width="50%"  border="1" colspan="2" class="invoice_table4 pull-right">
		<tr style='background-color:#375c7c;color:#fff;height:30px;'>
		 ...
		 <td><span class="pull-right" style="color:#3278b6"><b><?php echo $this->common->currency_decimal($this->common_model->calculate_currency($amount)); ?> <?php  echo $to_currency;  ?></b> </span></td>
...
<input id="auto_invoice_date_<?= $j ?>" name="auto_invoice_date[<?= $id ?>]" type='text' class='form-control col-md-3'  value="<?= $created_date ?>">
</td>
<td>
<textarea id="auto_invoice_description_<?= $j ?>" name="auto_invoice_description[<?= $id ?>]" style='height:35px;' class="form-control"><?php echo $value['description']; ?>  <?php echo $paypalid; ?></textarea>
</td>
<td><input type='text' id="auto_invoice_amount_<?= $j ?>" name="auto_invoice_amount[<?= $id ?>]" onkeyup="calculate()" class='form-control col-md-3'style="text-align:right;"  value="<?php echo $this->common->currency_decimal($this->common_model->calculate_currency($debit)); ?>"  >
</td>
</tr> 	
...
<input type="text" step="any"  id="invoice_amount_<?= $i ?>" name="invoice_amount_<?= $i ?>" onkeyup="calculate()" class='form-control col-md-3' style="text-align:right;" value="<?= $invoice_amt ?>"  >
</td>
...
<td style="text-align:right;"><div id='amount_val' name='amount_val'><?= $this->common->currency_decimal($this->common_model->calculate_currency($total_credit_sum)); ?></div>
</td>
</tr>
<!--
<tr style='background-color:#375c7c;color:#fff;height:30px;'>
<td colspan="2" style="text-align:right;">Total Tax:&nbsp;
</td>
<td style="text-align:right;"><div id='total_tax' name='total_tax'><?= $total_tax_dis; ?></div>
</td>
</tr>
-->
<?php
...
?>
	...
	<input type="hidden" id="total_tax_id_<?= $taxi; ?>" name="total_tax_id_<?= $taxi; ?>" value="<?= $tax_val['id']; ?>" >
...
<td style="text-align:right;"><div id='amount_val_final' name='amount_val_final'><?= $this->common->currency_decimal($this->common_model->calculate_currency($amount)); ?></div>
</td>
</tr>
