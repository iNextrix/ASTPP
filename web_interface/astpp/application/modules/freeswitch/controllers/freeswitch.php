...
class Freeswitch extends MX_Controller {
	...
	function fssipdevices_add($type = "") {
		...
		if ($account_data ['type'] == '-1' || $account_data ['type'] == '1') {
			...
			$data ['form'] = $this->form->build_form ( $this->freeswitch_form->get_freeswitch_form_fields (), '' );
	...
	function fssipdevices_edit($edit_id = '') {
		...
		if ($account_data ['type'] == '-1') {
			$data ['form'] = $this->form->build_form ( $this->freeswitch_form->get_freeswitch_form_fields ( $edit_id ), $account );
			$this->load->view ( 'view_freeswitch_add_edit', $data );
		} else {
			$data ['form'] = $this->form->build_form ( $this->freeswitch_form->get_freeswitch_form_fields ( $edit_id ), $account );
			$this->load->view ( 'view_freeswitch_customer_add_edit', $data );
	...
	function fssipdevices_save($user_flg = false) {
		...
		if (! $user_flg) {
			$data ['form'] = $this->form->build_form ( $this->freeswitch_form->get_freeswitch_form_fields ( $add_array ['id'] ), $add_array );
		} else {
		...
		if ($add_array ['id'] != '') {
			...
			if ($this->form_validation->run () == FALSE) {
				...
				$this->freeswitch_model->edit_freeswitch ( $add_array, $add_array ['id'] );
			...
			if ($this->form_validation->run () == FALSE) {
				...
				$this->freeswitch_model->add_freeswitch ( $add_array );
				echo json_encode ( array (
	...
	function customer_fssipdevices_save($user_flg = false) {
		$add_array = $this->input->post ();
		if (! $user_flg) {
			$data ['form'] = $this->form->build_form ( $this->freeswitch_form->get_freeswitch_form_fields (), $add_array );
		} else {
			$data ['form'] = $this->form->build_form ( $this->freeswitch_form->fsdevice_form_fields_for_customer ( $add_array ["accountcode"], $add_array ['id'] ), $add_array );
		}
		if ($add_array ['id'] != '') {
	...
	function fssipdevices_delete($id) {
		$this->freeswitch_model->delete_freeswitch_devices ( $id );
	...
	function livecall_report_json() {
		...
		foreach ( $calls as $key => $value ) {
			if (isset ( $value ['state'] ) && ($value ['state'] == 'CS_EXCHANGE_MEDIA' || $value ['state'] == 'CS_CONSUME_MEDIA' || $value['state'] == "CS_HIBERNATE" )) {
				...
				$diff_value = 0;
				if ($value ['callstate'] == 'ACTIVE') {
					$cmd = "api uuid_dump " . $value ['uuid'] . "";
					$fs_result = $this->freeswitch_model->reload_freeswitch ( $cmd );
					$fs_array = explode ( "\n", $fs_result );
					foreach ( $fs_array as $key => $sub_value ) {
						$sub_fs_array = explode ( ':', $sub_value );
						if (trim ( $sub_fs_array [0] ) == 'Caller-Channel-Answered-Time') {
		...
		if ($gateway_data ['id'] != '') {
			...
			if ($this->form_validation->run () == FALSE) {
				...
				if (preg_match ( '/\s/', $insert_arr ['name'] )) {
					echo json_encode ( array (
							"name_error" => "Gateway name must not have any space." 
					) );
					exit ();
				}
				/*
				...
				echo json_encode ( array (
						"SUCCESS" => $insert_arr ['name'] . " Gateway Updated Successfully!" 
				) );
				exit ();
			}
		} else {
			...
			if ($this->form_validation->run () == FALSE) {
				...
				echo json_encode ( array (
						"SUCCESS" => $insert_arr ['name'] . " Gateway Added Successfully!" 
				) );
				exit ();
			}
		}
	...
	function fssipprofile() {
		$data ['username'] = $this->session->userdata ( 'user_name' );
		$data ['page_title'] = gettext ( 'SIP Profile' );
		$data ['search_flag'] = true;
		$this->session->set_userdata ( 'advance_search', 0 );
	...
	function fsserver_add($type = "") {
		$data ['username'] = $this->session->userdata ( 'user_name' );
		$data ['flag'] = 'create';
		$data ['page_title'] = gettext ( 'Create Freeswich Server' );
	...
	function fsserver_edit($edit_id = '') {
		$data ['page_title'] = gettext ( 'Edit Freeswich Server' );
		$where = array (
				'id' => $edit_id 
		);
	...
	function fsserver_save() {
		...
		if ($add_array ['id'] != '') {
			...
			if ($this->form_validation->run () == FALSE) {
				...
				echo json_encode ( array (
						"SUCCESS" => " Freeswitch Server Updated Successfully!" 
				) );
				exit ();
			}
		} else {
			...
			if ($this->form_validation->run () == FALSE) {
				...
				echo json_encode ( array (
						"SUCCESS" => "Freeswitch Server Added Successfully!" 
				) );
				exit ();
			}
		}
