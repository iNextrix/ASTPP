...
class common {
	...
	function get_did_status($select, $table, $status) {
		return ($status ['status'] == 0) ? "<span class='label label-sm label-inverse arrowed-in' title='".gettext('Release')."'>".gettext('Active')."<span>" : "<span class='label label-sm' title='".gettext('Release')."'>".gettext('Inactive')."<span>";
	}
	...
	function account_number_icon($select = "", $table = "", $number) {
		...
		if ($account_res ['type'] == 0) {
			$return_value = " <span title='".gettext('Edit')."'>" . $account_res ['number'] . " </span>" . '<span class="badge badge-success float-left mt-1" title="'.gettext('Customer').'">'.gettext('Customer').'</span>';
		}
		if ($account_res ['type'] == 3) {
			$return_value = " <span title='".gettext('Edit')."'>" . $account_res ['number'] . " </span>" . '<span class="badge badge-primary float-left mt-1" title="'.gettext('Provider').'">'.gettext('Provider').'</span>';
		}
		if ($account_res ['type'] == - 1 || $account_res ['type'] == 2) {
			$return_value = " <span title='".gettext('Edit')."'>" . $account_res ['number'] . " </span>" . '<span class="badge badge-danger" title="'.gettext('Admin').'">'.gettext('Admin').'</span>';
		}
		if ($account_res ['type'] == 4) {
			$return_value = " <span title='".gettext('Edit')."'>" . $account_res ['number'] . " </span>" . '<span class="badge badge-secondary" title="'.gettext('Subadmin').'">'.gettext('Subadmin').'</span>';
		}
		return $return_value;
	}
	...
	function get_paid_status($select = "", $table = "", $status) {
		return gettext(($status == 1) ? "Paid" : "Unpaid");
	}
	function set_account_type_recharge($select = "", $table = "", $status) {
		return gettext(($status == 1) ? "Postpaid" : "Prepaid");
	}
	...
	function get_account_type($select = "", $table = "", $PTE) {
		return gettext(($PTE == 1) ? "Postpaid" : "Prepaid");
	}
	...
	function attachment_icons($select = "", $table = "", $attachement = "") {
		if ($attachement != "") {
			...
			foreach ( $array as $key => $val ) {
				...
				$str .= "<a href='" . $link . "' title='" . gettext($val) . "' class='btn btn-royelblue btn-sm'><i class='fa fa-paperclip fa-fw'></i></a>&nbsp;&nbsp;";
			}
			return $str;
		} else {
			return "";
	...
	function build_animap_button($button_params, $linkid) {
		$link = base_url () . $button_params->url . "" . $linkid;
		return '<a href="' . $link . '" class="btn btn-royelblue btn-sm animap_image" rel="facebox" title="'.gettext('ANI Map').'"><i class="fa fa-reorder fa-fw"></i></a>&nbsp;';
	}
	...
	function build_add_payment_button($url, $linkid) {
		$link = base_url () . $url . "" . $linkid;
		return '<a href="' . $link . '" style="cursor:pointer;color:#005298;" rel="facebox_medium" title="'.gettext('Refill').'">'.gettext('Payment').'</a>&nbsp;';
	}
	...
	function get_refill_coupon_used($select = '', $table = '', $status) {
		return $status ['status'] == 2 ? '<img src= "' . base_url () . 'assets/images/true.png" style="height:20px;width:20px;" title="'.gettext('Yes').'">' : '<img src= "' . base_url () . '/assets/images/false.png" style="height:20px;width:20px;" title="'.gettext('No').'">';
	}
	...
	function get_status_new($select = "", $table = "", $status = "") {
		return ($status ['status'] == 0) ? "<span class='label label-sm label-inverse arrowed-in' title='".gettext('Release')."'>".gettext('Active')."<span>" : "<span class='label label-sm' title='release'>".gettext('Inactive')."<span>";
	}
	...
	function optin_status($select = "", $table = "", $status) {
		...
		return  '<a href="'.$link.'" class="btn btn-royelblue btn-sm animap_image" rel="facebox" title="'.gettext('Optinon').'" onclick="return javascript:optinproduct(' . $status  . ')"><i class="fa fa-reorder fa-fw"></i></a>';
				
	}
	
	function check_did_available_reseller($select='', $table='', $number) {
		$link =  base_url () ."did/did_available_purchase/" . $number;
		return  '<a href="'.$link.'" <span  class="label label-sm label-inverse_blue arrowed_blue-in" rel="facebox" title="'.gettext('Release').'">'.gettext('Purchase').'<span></a>';
	}
	...
	function payment_status_icon($select = "", $table = "", $commission_status) {
		...
		if ($commission_status ['commission_status'] == "PAID") {
			$return_value = " <span title='".gettext('Paid')."'> </span>" . '<span class="badge badge-success float-center mt-1" title="'.gettext('Paid').'">'.gettext('Paid').'</span>';
		}
		if ($commission_status ['commission_status'] == "PENDING") {
			$return_value = " <span title='".gettext('Pending')."'> </span>" . '<span class="badge badge-primary float-center mt-1" title="'.gettext('Pending').'">'.gettext('Pending').'</span>';
		}
...
//HP: Permission changes 25-Jan-2019
	...
	function set_transaction_type($select,$table,$where){
		...
		$transaction_type = json_decode($where,true);
			...
			$set_transaction_type = " <span title='".gettext('Edit')."'>" . " </span>" . '<span class="badge badge-success" title="'.gettext('Admin').'">'.gettext('Sucess').'</span>';
		}
		if($transaction_type ['payment_status'] == "PENDING") {
			$set_transaction_type = " <span title='".gettext('Edit')."'>" . " </span>" . '<span class="badge badge-warning" title="'.gettext('Admin').'">'.gettext('Pending').'</span>';
		}
		if($transaction_type ['payment_status'] == "UNPAID") {
			$set_transaction_type = " <span title='".gettext('Edit')."'>" . " </span>" . '<span class="badge badge-danger" title="'.gettext('Admin').'">'.gettext('Fail').'</span>';
		}
