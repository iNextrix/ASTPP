...
$("#flex1").flexigrid({
    url: "<?php echo base_url();?>userReports/myReport_grid/",
    method: 'GET',
    dataType: 'json',
	colModel : [
<!--        {display: '<input type="checkbox" onclick="toggleChecked(this.checked)">', name: '', width: 20, align: 'center'},\
-->
	{display: 'Date', name: 'Date', width:140, sortable: false, align: 'center'},
        {display: 'CallerID', name: 'country', width: 130, sortable: false, align: 'center'},
        {display: 'Called Number', name: 'province', width: 130, sortable: false, align: 'center'},
        {display: 'BillSec', name: 'status', width: 70, sortable: false, align: 'center'},
        {display: 'Disposition', name: 'calls', width: 150, sortable: false, align: 'center'},
        {display: 'Debit', name: 'vm', width: 90, sortable: false, align: 'center'},        
	{display: 'Call Type', name: 'na', width: 100, sortable: false, align: 'center'},
		],
    buttons : [
...
$("#reseller_cdrs_search").click(function(){
	$.ajax({type:'POST', url: '<?=base_url()?>userReports/myReport_search', data:$('#form9').serialize(), success: function(response) {
    $('#flex1').flexReload();
}});
	});
	
	$("#id_reset").click(function(){
		$.ajax({url:'<?=base_url()?>userReports/clearsearchfilter_myReports', success:function(){
		$('#flex1').flexReload();	}
		});
	});
...
{
	window.location = '<?php echo base_url();?>userReports/clearsearchfilter_myReports/';
}
...
<div class="portlet ui-widget ui-widget-content ui-helper-clearfix ui-corner-all" id="searchbar">                        
	  ...
	  $(document).ready(function() {
		...
		$("#reseller_cdr_to_date").datetimepicker({ dateFormat: 'yy-mm-dd' });			
          ...
          <form action="<?=base_url()?>userReports/myReport_search" id="form9" name="form9" method="POST" enctype="multipart/form-data" style="display:block">
            <input type="hidden" name="ajax_search" value="1">
         <input type="hidden" name="advance_search" value="1">
         <ul style="list-style:none;">
          <fieldset  >
            <legend><span style="font-size:14px; font-weight:bold; color:#000;">Search My Call Report </span></legend>
            	<li>
...
<div class="portlet ui-widget ui-widget-content ui-helper-clearfix ui-corner-all">                        
    <div class="portlet-header ui-widget-header">My Call Detail Report<span class="ui-icon ui-icon-circle-arrow-s"></span></div>
