...
$("#flex1").flexigrid({
    url: "<?php echo base_url();?>adminReports/reseller_list/grid/",
    method: 'GET',
    dataType: 'json',
	colModel : [
...
$("#id_filter").click(function(){
	...
	flex_url = "<?php echo base_url();?>adminReports/reseller_list/grid/"+account+"/"+company+"/"+fname+"/"+lname;
	//alert(flex_url);
...
function sendValue(selvalue, othervalue){
	window.opener.document.form2.reseller.value = selvalue;
	if(othervalue && window.opener.document.form2.accountcode){
			window.opener.document.form2.accountcode.value = othervalue;
	}
	window.close();
}
