...
$("#flex1").flexigrid({
    url: "<?php echo base_url();?>rates/pricelists_grid/",
    method: 'GET',
    dataType: 'json',
	colModel : [
<!--        {display: '<input type="checkbox" onclick="toggleChecked(this.checked)">', name: '', width: 20, align: 'center'},\
-->
		{display: 'Pricelist Name', name: 'Number', width: 100, sortable: false, align: 'center'},
        {display: 'Default Increment', name: 'country', width: 150, sortable: false, align: 'center'},
        {display: 'Markup in 1/100 of 1\%', name: 'province', width: 150, sortable: false, align: 'center'},
        {display: 'Number of routes', name: 'city', width: 100, sortable: false, align: 'center'},
...
$("#rates_pricelist_search").click(function(){
	$.ajax({type:'POST', url: '<?=base_url()?>rates/pricelist_search', data:$('#search_form11').serialize(), success: function(response) {
    $('#flex1').flexReload();
		}});
	});
	
	$("#id_reset").click(function(){
		$.ajax({url:'<?=base_url()?>rates/clearsearchfilter/', success:function(){
		$('#flex1').flexReload(); }
		});
	});
...
function add_button()
{
    jQuery.facebox({ ajax: '<?php echo base_url();?>rates/pricelists/add'});
}

function clear_filter()
{
	window.location = '<?php echo base_url();?>rates/clearsearchfilter/';
}
...
<div class="portlet ui-widget ui-widget-content ui-helper-clearfix ui-corner-all" id="searchbar">                        
	     ...
	     <form action="<?=base_url()?>rates/pricelist_search" id="search_form11" name="form11" method="POST" enctype="multipart/form-data" style="display:block">
         ...
         <ul style="list-style:none;">
          <fieldset  >
            ...
            <li>
                ...
                <input size="20" class="text field" name="pricelist_name"> &nbsp;
                <select name="pricelist_name_operator"  class="field select">
                ...
                <span>
                 ...
                 <input size="20" class="text field" name="default_increment"> &nbsp;
                 <select name="default_increment_operator"  class="field select" style="width:132px;">
        ...
        <input type="button" class="ui-state-default float-right ui-corner-all ui-button" name="action" value="Search" id="rates_pricelist_search" style="margin-right:22px;" />
        <br><br>
         </form>             
          </div>
        </div>
<div class="portlet ui-widget ui-widget-content ui-helper-clearfix ui-corner-all">                        
        <div class="portlet-header ui-widget-header">Price List<span class="ui-icon ui-icon-circle-arrow-s"></span></div>
