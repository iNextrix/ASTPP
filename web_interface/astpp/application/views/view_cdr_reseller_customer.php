...
$("#flex1").flexigrid({
    url: "<?php echo base_url();?>cdrReports/customerReport_grid/",
    method: 'GET',
    dataType: 'json',
	colModel : [
<!--        {display: '<input type="checkbox" onclick="toggleChecked(this.checked)">', name: '', width: 20, align: 'center'},\
-->
	{display: 'Date', name: 'Date', width:120, sortable: false, align: 'center'},
        {display: 'CallerID', name: 'country', width: 110, sortable: false, align: 'center'},
        {display: 'Called Number', name: 'province', width: 110, sortable: false, align: 'center'},
        ...
        {display: 'Debit', name: 'vm', width: 70, sortable: false, align: 'center'},
        {display: 'Cost', name: 'failed', width: 70, sortable: false, align: 'center'},
...
$("#customer_cdrs_search").click(function(){
	$.ajax({type:'POST', url: '<?=base_url()?>cdrReports/customerReport_search', data:$('#form9').serialize(), success: function(response) {
    $('#flex1').flexReload();
}});
	});
	
	$("#id_reset").click(function(){
		$.ajax({url:'<?=base_url()?>cdrReports/clearsearchfilter_customerReports', success:function(){
		$('#flex1').flexReload();	}
		});
	});
...
{
	window.location = '<?php echo base_url();?>cdrReports/clearsearchfilter_customerReports/';
}
...
<div class="portlet ui-widget ui-widget-content ui-helper-clearfix ui-corner-all" id="searchbar">                        
	  ...
	  $(document).ready(function() {
		...
		$("#customer_cdr_to_date").datetimepicker({ dateFormat: 'yy-mm-dd' });			
          ...
          <form action="<?=base_url()?>cdrReports/customerReport_search" id="form9" name="form9" method="POST" enctype="multipart/form-data" style="display:block">
            <input type="hidden" name="ajax_search" value="1">
         <input type="hidden" name="advance_search" value="1">
         <ul style="list-style:none;">
          <fieldset  >
            <legend><span style="font-size:14px; font-weight:bold; color:#000;">Search Customer Call Report </span></legend>
            	<li>
                     
                      <div class="float-left" style="width:30%">
                 	 <span>
                     	   <label> From date & Time :</label>
           				   <input size="20" class="text field" name="start_date" id="customer_cdr_from_date"> &nbsp;<img src="<?=base_url()?>images/calendar.png" border="0"> 
                     </span>
                     </div>
                 	 ...
                 	 <span>
            				...
            				<input size="20" class="text field" name="end_date" id="customer_cdr_to_date"> &nbsp;<img src="<?=base_url()?>images/calendar.png" border="0">        
                     </span>
                     </div>
                ...
                <div class="float-left" style="width:30%">
                	...
                	<input size="20" class="text field" name="customer" id="customer">
        ...
        <input type="button" class="ui-state-default float-right ui-corner-all ui-button" name="action" value="Search" id="customer_cdrs_search" style="margin-right:22px;" />
        <br><br>
...
<div class="portlet ui-widget ui-widget-content ui-helper-clearfix ui-corner-all">                        
    <div class="portlet-header ui-widget-header">Customer Call Detail Report<span class="ui-icon ui-icon-circle-arrow-s"></span></div>
...
<div style="float:right;"><strong><a href="/cdrReports/export_cdr_customer_xls">Export XLS <img src="/images/file_tree/xls.png" alt='XLS'/></a> | <a href="/cdrReports/export_cdr_customer_pdf">Export PDF <img src="/images/file_tree/pdf.png" alt='PDF'/></a></strong></div>
<br/><br/>
<?php 
	//echo $form;
?>
