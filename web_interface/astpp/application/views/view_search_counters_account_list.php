...
$("#flex1").flexigrid({
    url: "<?php echo base_url();?>accounts/search_counters_account_list/grid/",
    method: 'GET',
    dataType: 'json',
	colModel : [
...
$("#id_filter").click(function(){
	...
	flex_url = "<?php echo base_url();?>accounts/search_counters_account_list/grid/"+account+"/"+company+"/"+fname+"/"+lname;
	//alert(flex_url);
...
function sendValue(selvalue, othervalue){
	window.opener.document.form15.account_number.value = selvalue;
	if(othervalue && window.opener.document.form15.accountcode){
			window.opener.document.form15.accountcode.value = othervalue;
	}
	window.close();
}
