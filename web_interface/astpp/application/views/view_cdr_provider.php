...
$("#flex1").flexigrid({
    url: "<?php echo base_url();?>cdrReports/providerReport_grid/",
    method: 'GET',
    dataType: 'json',
	colModel : [
...
-->
	{display: 'Date', name: 'Date', width:120, sortable: false, align: 'center'},
        {display: 'CallerID', name: 'country', width: 120, sortable: false, align: 'center'},
...
$("#provider_cdrs_search").click(function(){
	$.ajax({type:'POST', url: '<?=base_url()?>cdrReports/providerReport_search', data:$('#form7').serialize(), success: function(response) {
    $('#flex1').flexReload();
}});
	});
	
	$("#id_reset").click(function(){
		$.ajax({url:'<?=base_url()?>cdrReports/clearsearchfilter_providerReports', success:function(){
		$('#flex1').flexReload();	}
		});
	});
...
{
	window.location = '<?php echo base_url();?>cdrReports/clearsearchfilter_providerReports/';
}
...
<div class="portlet ui-widget ui-widget-content ui-helper-clearfix ui-corner-all" id="searchbar">                        
            ...
            <div class="portlet-content" id="search_bar">
              ...
              <script>
	  $(document).ready(function() {
		$("#provider_cdr_from_date").datetimepicker({ dateFormat: 'yy-mm-dd' });		
		$("#provider_cdr_to_date").datetimepicker({ dateFormat: 'yy-mm-dd' });			
	  });
	  </script> 
          <form action="<?=base_url()?>cdrReports/providerReport_search" id="form7" name="form7" method="POST" enctype="multipart/form-data" style="display:block">
            <input type="hidden" name="ajax_search" value="1">
         <input type="hidden" name="advance_search" value="1">
         <ul style="list-style:none;">
          <fieldset  >
            <legend><span style="font-size:14px; font-weight:bold; color:#000;">Search provider Call Report </span></legend>
            	<li>
                     
                      <div class="float-left" style="width:30%">
                 	 <span>
                     	   <label> From date & Time :</label>
           				   <input size="20" class="text field" name="start_date" id="provider_cdr_from_date"> &nbsp;<img src="<?=base_url()?>images/calendar.png" border="0"> 
                     </span>
                     </div>
                 	 ...
                 	 <span>
            				...
            				<input size="20" class="text field" name="end_date" id="provider_cdr_to_date"> &nbsp;<img src="<?=base_url()?>images/calendar.png" border="0">        
                     </span>
                     </div>
        ...
        <input type="button" class="ui-state-default float-right ui-corner-all ui-button" name="action" value="Search" id="provider_cdrs_search" style="margin-right:22px;" />
        <br><br>
...
<div style="float:right;"><strong><a href="/cdrReports/export_cdr_provider_xls">Export XLS <img src="/images/file_tree/xls.png" alt='XLS'/></a> | <a href="/cdrReports/export_cdr_provider_pdf">Export PDF <img src="/images/file_tree/pdf.png" alt='PDF'/></a></strong></div>
<br/><br/>
<?php 
	//echo $form;
?>
