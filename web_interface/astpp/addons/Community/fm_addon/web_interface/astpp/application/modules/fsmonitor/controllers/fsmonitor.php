...
class Fsmonitor extends CI_Controller {
	...
	$data['page_title'] = gettext("Authentication");	
	$data['type'] = "Sip";
	...
	$data['username'] = $this->session->userdata('user_name');
	$data['page_title'] = gettext("Opensip Devices");
	$query = $this->db_model->getSelect("*", "freeswich_servers", "");
        $data['fs_data'] = $query->result_array();
...
/*    function sip_devices_json($id=''){
    ...
    function fs_cli() {
		$data['username']   = $this->session->userdata('user_name');
		$data['page_title'] = gettext('Freeswitch CLI');
		$data['response']   = '';
		$data['command']    = '';
		$query              = $this->db_model->getSelect("*", "freeswich_servers", "");
	    $data['fs_data']    = $query->result_array();
	   ...
	   $json_data['page'] = 1;
           ...
           if(count($response_arr)>0){
		    ...
		    if($response_value['state'] == 'FAIL_WAIT'){
		        $url=base_url();
		        $btn = '<span class="label_red label-sm_red label-inverse_red arrowed-in_red" style="padding-top:4px;">'.gettext('FAIL_WAIT').'</span>';
		    }
		    elseif($response_value['state'] == 'REGED'){
		        $url=base_url();
		        $btn = '<span class="label label-sm label-inverse arrowed-in" style="padding-top:4px;">'.gettext('REGED').'</span>';
		    }
		    else{ 
			$url=base_url();
		 	$btn = '<span class="label label-sm label-inverse arrowed-in"  style="padding-top:4px;">'.gettext('NOREG').'</span> ';
		   } 
		   $edit_action ='<a href="../gateways_popup/'.$response_value["name"].'/'.$id.'" class="btn btn-royelblue btn-sm"  rel="facebox_medium" title="'.gettext('Edit').'">&nbsp;<i class="fa fa-pencil-square-o fa-fw"></i></a>';
	 	   $action=   $edit_action;
                   $json_data['rows'][] = array('cell'=>array(
		...
		$edit_action ='<a href="../gateways_popup/'.$response_final_arr["name"].'/'.$id.'" class="btn btn-royelblue btn-sm"  rel="facebox_medium" title="'.gettext('Edit').'">&nbsp;<i class="fa fa-pencil-square-o fa-fw"></i></a>';
		$action= $edit_action;
                $json_data['rows'][] = array('cell'=>array(
                                     ...
                                     gettext($response_final_arr["status"]),
				     $btn,
				     $action,
                                 ));
            }
	...
	if($data_result == ''){
		$data['page_title'] = gettext('Gateway');
		$data['fail'] = 1;
	}
	else{
		$data =array();
