...
UPDATE `userlevels` SET `module_permissions` = concat( `module_permissions`, ',', (  SELECT max( `id` ) FROM `menu_modules` where `module_url` = "switch_monitoring/sip_devices/" ) ) WHERE `userlevelid` = 1;
INSERT INTO `roles_and_permission` (`id`, `login_type`, `permission_type`, `menu_name`, `module_name`, `sub_module_name`, `module_url`, `display_name`, `permissions`, `status`, `creation_date`, `priority`) VALUES (NULL, '1', '0', 'switch', 'switch_monitoring', '', 'sip_devices', 'Registered SIP Device', '[\"main\",\"list\"]', '0', '2019-01-25 09:01:05', '6.34');
