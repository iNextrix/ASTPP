...
class User_cdrs extends Account
{
	protected $postdata = "";
	...
	private function _user_cdrs_list()
	{
		...
		if(isset($object_where_params['debit']) && !empty($object_where_params['debit']) ){
			$object_where_params_debit['debit'] = $this->common_model->add_calculate_currency($object_where_params['debit'],$to_currency,$from_currency, true, false);
			$this->db->where($object_where_params_debit);
			unset($object_where_params['debit']);
		}
		...
		foreach($object_where_params as $object_where_key => $object_where_value) {
			if($object_where_value != '') {	
				if(isset($object_where_key) && $object_where_key == 'destination'){
					$this->db->like('notes', $object_where_params['destination'] );
				}
		...
		if(!empty($where)){
			unset($where['destination'],$where['duration']);
			$this->db->like($where, $object_where_params );
		}
		// Kinjal issue no 3556
		$call_direction= array('inbound', 'outbound');	
		$this->db->where_in('call_direction', $call_direction);
		// END
		$this->db->where('accountid' , $this->postdata['id']);
		$this->db->order_by("callstart", "desc");
		$this->db->limit($no_of_records, $start);
        $this->db->select('callstart,sip_user,calltype,callerid,call_direction,callednum,notes,billseconds,disposition,debit');
		...
		foreach ($cdrs_info as $key => $cdrs_value) {  
            ...
            unset($cdrs_value['notes'],$cdrs_value['billseconds']);
			$cdrsinfo[] =$cdrs_value;
		}
    	if (!empty($cdrsinfo)) {
			$this->response ( array (
