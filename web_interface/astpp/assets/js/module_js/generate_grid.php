...
function get_alert_msg(id)
{
    //confirm_string = 'are you sure to delete?';
      confirm_string = '<?=gettext('Are you sure want to delete?')?>';
...
function build_grid_reports(grid_id,destination,collumn_arr,buttons){
    ...
    $("#"+grid_id).flexigrid({
        ...
        resizable: true,
        title: '',
        pagetext: '<? echo gettext('Page');?>',
        outof: 'of',
        nomsg: '<? echo gettext('No Records');?>',
...
function build_collumns(collumn_arr){
    ...
    for (var i = 0; i < col_field.length; i++) {
	    ...
	    if(collumn_property[6] =='false' && collumn_property[0] == '<? echo gettext('Action'); ?>'){
		continue;
	    }
	    if(collumn_property[7] =='false' || collumn_property[0] == '<? echo gettext('Action'); ?>'){
		searchflg = false;
	    }else if(collumn_property[0] == '<? echo gettext('Account'); ?>' && collumn_property[7]== 'build_concat_string' && collumn_property[9]=='false'){
            searchflg = false;
        }else{
	      searchflg = true;
	    }
...
//     alert(value);
    if (value){
	    ...
	    success: function(response) {
		document.cookie = "lang="+value+";expires=Mon, 7 Oct 2030 12:00:00 UTC;path=/";
		location.reload(true);
	    },
	});
    }
