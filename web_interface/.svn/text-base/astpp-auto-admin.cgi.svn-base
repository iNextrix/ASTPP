...
if ( $params->{reseller} ne "null" && $params->{reseller}) {
	my $config_reseller = &load_config_reseller_db($astpp_db,$config,$params->{username});
...
#################  Calling Card Support Starts Here ################
	...
	while ( $count < $params->{quantity} ) {
		$count++;
		my ( $cc, $pin ) =
		  &add_callingcard( $astpp_db, $config, $branddata,
	...
	$status = &transfer_funds($astpp_db,$config,$params->{emptycard}, $params->{emptycardpin}, $params->{destcard}, $params->{destcardpin});
        if ($status == 0) {
