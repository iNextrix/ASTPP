...
#define MAX_ACCOUNT_COUNT 10  // for now assuming reseller max depth is 10
...
typedef struct {
	...
	int initial_increment_done;  /* Set to 1 when we charged initial increment so that we can charge next increment rate*/
	int total_bill_sec;
} nibble_data_t;
...
static switch_status_t do_billing(switch_core_session_t *session)
{
	...
	switch_time_t billmicrosecs = 0;
	switch_time_t tempmicrosecs = 0;
	/* Local vars */
	...
	const char *billrate;
	const char *billrates;
	
	const char *billincrement;
	
	const char *billaccount;
	const char *billaccounts;
	
	const char *billconnectcosts;
	const char *bill_init_incs;
	const char *bill_incs;
	...
	char *billaccount_strings[MAX_ACCOUNT_COUNT] = { 0 };
	char *billrate_strings[MAX_ACCOUNT_COUNT] = { 0 };
	char *billconnectcost_strings[MAX_ACCOUNT_COUNT] = { 0 };
	char *bill_init_inc_strings[MAX_ACCOUNT_COUNT] = { 0 };
	char *bill_inc_strings[MAX_ACCOUNT_COUNT] = { 0 };
	
	switch_time_t initail_chargedunits = 0;
	switch_time_t chargedunits = 0;
	...
	billrates = switch_channel_get_variable(channel, globals.var_name_rate);
	billincrement = switch_channel_get_variable(channel, "nibble_increment");
	billaccounts = switch_channel_get_variable(channel, globals.var_name_account);
	
	bill_init_incs = switch_channel_get_variable(channel, "nibble_init_inc");
	bill_incs = switch_channel_get_variable(channel, "nibble_inc");
	billconnectcosts = switch_channel_get_variable(channel, "nibble_connectcost");
	...
	switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG, "BILLRATES %s BILLACCOUNTS %s BILLCONNECT COSTS %s\n", billrates,
					  billaccounts, billconnectcosts);



	/* Return if there's no billing information on this session */
	if (!billrates || !billaccounts) {
		return SWITCH_STATUS_SUCCESS;
	}

	/* Parsing billrates*/
	switch_separate_string((char*)billrates, ',', billrate_strings, sizeof(billrate_strings) / sizeof(billrate_strings[0]));
	/* Parsing bill accounts*/
	switch_separate_string((char*)billaccounts, ',', billaccount_strings, sizeof(billaccount_strings) / sizeof(billaccount_strings[0]));
	/* Parsing connect costs*/
	switch_separate_string((char*)billconnectcosts, ',', billconnectcost_strings, sizeof(billconnectcost_strings) / sizeof(billconnectcost_strings[0]));
	/* Parsing initial increments */
	switch_separate_string((char*)bill_init_incs, ',', bill_init_inc_strings, sizeof(bill_init_inc_strings) / sizeof(bill_init_inc_strings[0]));
	/* Parsing increments */
	switch_separate_string((char*)bill_incs, ',', bill_inc_strings, sizeof(bill_inc_strings) / sizeof(bill_inc_strings[0]));
	
	
	for(i = 0; i < MAX_ACCOUNT_COUNT; i++){
		if(!billaccount_strings[i])
			break;
			
		switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG, "Parsed rate %s Parsed account %s Parsed connectcost %s Parsed initial inc %s Parsed increment %s\n", 
					  billrate_strings[i],
					  billaccount_strings[i],
					  billconnectcost_strings[i],
					  bill_init_inc_strings[i],
					  bill_inc_strings[i])
					  ;
	}
	...
	if (profile->times->answered < 1) {
		...
		for(i = 0; i < MAX_ACCOUNT_COUNT; i++){
			...
			billaccount = billaccount_strings[i];
			switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG, "Not billing %s - call is not in answered state But still checking if call can go through\n", billaccount);
			billrate = billrate_strings[i];
			minimum_charge = atof(billconnectcost_strings[i]);
	...
	if (globals.mutex) {
		...
		nibble_data->total_bill_sec+=(int) ((ts - nibble_data->lastts) / 1000000);
		...
		if (ts - nibble_data->lastts >= 0) {
			/* Starting bill all accounts with their respective costs*/
			billmicrosecs = billsecs * 1000000;//am i correct? I do not want micro second precision.
			for(i = 0; i < MAX_ACCOUNT_COUNT; i++){
				
				if(!billaccount_strings[i])
					break;
				
				billrate = billrate_strings[i];// get corresponding billrate for ith account
				billaccount = billaccount_strings[i]; // get ith account to bill;
				
				switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG, "Attempting to bill at $%s per minute to account %s\n", billrate,
					  billaccount);
					  
				initail_chargedunits = 0;
				chargedunits = 0;
				billamount = 0;
				tempmicrosecs = billmicrosecs;// save billed duration in temp variable because we will tinker with temp variable to calculate increment
				if(nibble_data->initial_increment_done == 0){
					
					billamount = atof(billconnectcost_strings[i]);//charging connectcost
					initail_chargedunits = atol(bill_init_inc_strings[i]) * 1000000;//track duration of billing
					billamount += (atof(billrate) / 1000000 / 60) * initail_chargedunits - nibble_data->bill_adjustments;//charging initial cost
					tempmicrosecs = tempmicrosecs - initail_chargedunits;
					

					
				}
				if (tempmicrosecs > 0) {
					billincrement = bill_inc_strings[i];
					chargedunits = (tempmicrosecs) / 1000000 <= atol(billincrement) ? atol(billincrement) * 1000000 : (switch_time_t)(ceil((tempmicrosecs) / (atol(billincrement) * 1000000.0))) * atol(billincrement) * 1000000;
					billamount += (atof(billrate) / 1000000 / 60) * chargedunits - nibble_data->bill_adjustments;
					
					
				} 
				
				/*IMPORTANT SECTION STARTS*/
				if(i == 0){
					/*i=0 always true for low node like customers. so we are tracking it by 0 and mark the timestamp billed for it*/
					nibble_data->lastts += chargedunits+initail_chargedunits;
				}
			...
			nibble_data->initial_increment_done = 1;// marking that we have done calculate all initial cost for node and their parents.
...
static switch_status_t sched_billing(switch_core_session_t *session)
{
	...
	switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_INFO, "We are in scheduled billing\n");
	/* Variables kept in FS but relevant only to this module */
	billrate = switch_channel_get_variable(channel, "nibble_account");
	billaccount = switch_channel_get_variable(channel, "nibble_rate");
...
static switch_status_t process_hangup(switch_core_session_t *session)
{
	...
	do_billing(session);
	switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_INFO, "UUID of session %s\n",switch_core_session_get_uuid(session));
	switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_INFO, "Processing hangup\n");
	/*billaccount = switch_channel_get_variable(channel, globals.var_name_account);
